...
import locators.QueryPageLocator;
...
public class ResultQueryPO {
    ...
    private SelenideElement nextQueryButton = $(By.id(QueryPageLocator.NEXT_BUTTON_PAGE.getPath()));

    public ResultQueryPO getAddressComponents() {
        if (listOfResultQuery == null) {
            ElementsCollection resultRoots = $$x(QueryPageLocator.LIST_OF_RESULTS.getPath());
